...
def find_authors_nationalities():
    author = Author.objects.exclude(nationality=None)
    return '\n'.join(f"{x.first_name} {x.last_name} is {x.nationality}" for x in author)

def order_books_by_year():
    books = Book.objects.all().order_by('publication_year', 'title')
    return '\n'.join(f"{x.publication_year} year: {x.title} by {x.author}" for x in books)
